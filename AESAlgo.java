...
public class AESAlgo {
	...
	/**
	 * S-Box used for sub bytes function
	 */
	...
	/**
	 * Inverse SBOX table used for inverse SubBytes function
	 */
	...
	/**
	 * table for inverse MixColumns function
	 */
	...
	/**
	 * RCon look up table used by Key Expansion algorithm to generate 44 words for AES algorithm
	 */
	...
	/**
	 *This method is used for encrypt and decrypt plain text
	 *@param line which indicates the input plain text 
	 *@param key indicates the key entered by the user for encryption and decryption 
	 */
	public String aesAlgo(String line , String key) throws IOException 
	{
		...
		if (line.matches("[0-9A-F]+")) 
		{
			...
			addRoundKey(state, subKey(keymatrix, 0)); // first part of Key Expansion
			for (int i = 1; i < numRounds; i++) {
				subBytes(state); //implements the SubBytes function.
				shiftRows(state); //implements ShiftRows function.
				mixColumns(state); //implements MixColumns function.
			...
			String output = MatrixToString(state);
			System.out.println("Encrypted data from AES algorithm >> " + output);
			decryptedData = decryptFunction(output, key);
	...
	public static String decryptFunction(String line, String key) {
		...
		output = MatrixToString(state);
		System.out.println("Decrypted Data from AES algorithm >> " + output);
		return output;
	...
	private static int mcCalc(int a, int b) //Helper method for mcHelper
	{
		if (a == 1) {
			...
			return MixColumns.mc2[b / 16][b % 16];
		} else if (a == 3) {
			return MixColumns.mc3[b / 16][b % 16];
		}
		return 0;
	}
	...
	{
		if (a == 9) {
			return MixColumns.mc9[b / 16][b % 16];
		} else if (a == 0xb) {
			return MixColumns.mc11[b / 16][b % 16];
		} else if (a == 0xd) {
			return MixColumns.mc13[b / 16][b % 16];
		} else if (a == 0xe) {
			return MixColumns.mc14[b / 16][b % 16];
		}
		return 0;
	}
...
{
    /* 
     * These look up tables are used based on what a certain number is multiplied by for eg: if some number is multiplied by 3
     * then use mc3 table. 
     */
